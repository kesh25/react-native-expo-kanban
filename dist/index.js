...
const ReactNativeKanbanBoard = (props) => {
    ...
    const columnsHorizontalScrollRef = useAnimatedRef();
    ...
    const animatedStyle = useAnimatedStyle(() => {
        return {
            ...
            transform: [
                ...
                {
                    rotate: interpolate(dragX.value, [-scrollTriggerWidth, 0, scrollTriggerWidth], [12, 0, 12], "extend") + "deg",
                },
                {
                    scale: interpolate(dragX.value, [-scrollTriggerWidth, 0, scrollTriggerWidth], [1.12, 1, 1.12], "extend"),
                },
            ],
        };
    }, [dragItem]);
    const renderColumn = ({ item: columnData, index: i, }) => {
        const isPotentiallyBeingMoveTo = dragItem?.columnIndex !== undefined &&
            i !== dragItem.columnIndex &&
            toColumnIndex === i;
        const isItemInFocusedColumn = i === toColumnIndex;
        const renderCard = ({ item }) => {
            const isBeingDragged = dragItem?.id === item.id;
            return (<DataCard disableScroll={disableScroll} setDragCard={setDragCard} renderItem={props.renderItem} isDraggable={!dragItem && isItemInFocusedColumn} itemColumnIndex={i} item={item} isBeingDragged={isBeingDragged}/>);
        };
    ...
    const onMomentumScrollEnd = (event) => {
        const columnIndex = event.nativeEvent.contentOffset.x === 0
            ? 0
            : Math.round((event.nativeEvent.contentOffset.x + marginAlign) /
                columnContainerWidth);
        updateCurrentColumnIndex(columnIndex);
        setToColumnIndex(columnIndex);
    };
    return (<DragContextValue.Provider value={{
            setDragCard,
      ...
      <GestureDetector gesture={pan}>
        <View>
          <Animated.FlatList ref={columnsHorizontalScrollRef} horizontal pagingEnabled snapToInterval={columnContainerWidth} snapToAlignment={"center"} decelerationRate={"fast"} onMomentumScrollEnd={onMomentumScrollEnd} data={props.columnData} renderItem={renderColumn} contentContainerStyle={props.contentContainerStyle} style={props.containerStyle}/>

          {dragItem?.props && (<Animated.View style={animatedStyle}>
              {props.renderItem(dragItem.props, true)}
            </Animated.View>)}
        </View>
      </GestureDetector>
